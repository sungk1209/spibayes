...
# | CREATED BY:  Kay Sung     
# *----------------------------------------------------------------
# | PURPOSE: Run multiple spibayes sample only
# |
# |
# *------------------------------------------------------------------
...
bayes_tensor <- function(id) {
  ...
  knots_year <- seq(1920,2018,length=n_knots_year)
  ...
  fitting_df <- accum_df %>%
    ...
    mutate(roll_mean_3_notna = rollsumr(x=!is.na(prcp), k=n_roll, fill=NA, na.rm=TRUE)) %>%
  ...
  ###  Compile the model
  ###########################################################################
  ### I still don't know how to specify an output directory for compiled model
 ...
 # b_mean_disp <-fit_quick$mle("b_disp_jdate")
  b_disp_jate <-fit_quick$mle("b_disp_jdate")
  b_disp_year <-fit_quick$mle("b_disp_year")
  
  b_para <- c(b_0_mean, b_mean_jdate,b_mean_year,b_mean_tensor,b_disp_jate, b_disp_year)           
  
  filename <- paste0(output_path,id,"_para.rda")
  save(b_para, file = filename)
...
#   
#  p <- ggplot(plot_df, aes(x=jdate)) + geom_line(aes(y=precip, group = year)) + geom_line(aes(y=exp(mean_est_full), group = year)) + scale_colour_viridis() #+ geom_line(aes(y=exp(mean_init)), colour = "blue")
...
station_list <- station_list[,1:3]

#source(file.path(here_path,"/functions/bayes_tensor.R"))
numCores <- parallel::detectCores() - 1
plan(multicore)

future_map(station_list$id, bayes_tensor)
